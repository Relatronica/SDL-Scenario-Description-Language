...
class SeededRNG {
  ...
  normalSample(mean: number, std: number): number {
    const u1 = this.next();
    const u2 = this.next();
...
function sampleDistribution(dist: DistributionExpression, baseValue: number, rng: SeededRNG): number {
...
function interpolateTimeseries(
  ...
  // Before first entry
  if (targetYear <= entries[0].year) return entries[0].value;
...
export function simulate(
  ...
  if (simNode) {
    ...
    if (simNode.method) config.method = simNode.method;
    if (simNode.seed) config.seed = simNode.seed;
  ...
  const assumptions = new Map<string, AssumptionNode>();
  const parameters = new Map<string, ParameterNode>();
  const branches: BranchNode[] = [];
  const impacts = new Map<string, ImpactNode>();

  for (const decl of scenario.declarations) {
    switch (decl.type) {
      case 'Variable': variables.set(decl.name, decl); break;
      case 'Assumption': assumptions.set(decl.name, decl); break;
      case 'Parameter': parameters.set(decl.name, decl); break;
      case 'Branch': branches.push(decl); break;
      case 'Impact': if (decl.name !== '_list') impacts.set(decl.name, decl); break;
    }
  }

  // Prepare result collectors
  const variableCollectors = new Map<string, Map<number, number[]>>();
  const impactCollectors = new Map<string, Map<number, number[]>>();
  const branchActivations = new Map<string, number>();
  ...
  const variableTimeseries = new Map<string, { year: number; value: number }[]>();
  for (const [name, varNode] of variables) {
    const entries: { year: number; value: number }[] = [];
    for (const entry of varNode.timeseries) {
      const val = extractNumber(entry.value as ExpressionNode);
  ...
  for (let run = 0; run < config.runs; run++) {
    ...
    const state = new Map<string, Map<number, number>>();
    ...
    for (const varName of causalGraph.topologicalOrder) {
      ...
      for (const ts of timesteps) {
        ...
        if (tsEntries.length > 0) {
          baseValue = interpolateTimeseries(
            tsEntries,
            year,
      ...
      for (const ts of timesteps) {
        ...
        if (evaluateCondition(branch.condition, state, year)) {
          if (rng.next() < (branch.probability ?? 0.5)) {
            branchActivations.set(
              branch.name,
              (branchActivations.get(branch.name) || 0) + 1
            );
            break;
          }
        }
  ...
  // ── Aggregate results ──
  const variableResults = new Map<string, VariableResult>();
  ...
  const impactResults = new Map<string, ImpactResult>();
  ...
  return {
    ...
    config: scenario.declarations.find(d => d.type === 'Simulate') as SimulateNode || {
...
function generateTimesteps(start: DateValue, end: DateValue, resolution: string): Date[] {
  const steps: Date[] = [];
...
function evaluateExpression(
  ...
  switch (expr.type) {
    ...
    case 'Identifier': {
      ...
      if (varState) return varState.get(year) ?? 0;
      return 0;
    }
...
function evaluateBuiltinFunction(name: string, args: number[]): number {
  switch (name) {
...
function computeDistribution(samples: number[], percentiles: number[]): DistributionResult {
  if (samples.length === 0) {
    return {
  ...
  const percMap = new Map<number, number>();
...
function checkConvergence(
  collectors: Map<string, Map<number, number[]>>,
  threshold: number
): boolean {
  for (const [, yearMap] of collectors) {
    for (const [, samples] of yearMap) {
      if (samples.length < 200) return false;
